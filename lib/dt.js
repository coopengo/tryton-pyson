...
DTMixin.prototype.resolve = function (context, items, deltas, fn) {
  var keys = [].concat(items, deltas);
  var resolved = _.mapObject(_.pick(this, keys),
    (v) => Pyson.resolve(v, context));
  ...
  delta = _.filter(delta, (v) => v !== null && v !== 0);
  delta = _.pairs(delta);
  delta = _.map(delta, (p) => [items[_.indexOf(deltas, p[0])], p[1]]);
  dt.add(_.object(delta));
  return dt;
};
//
// exports
